...
- `{tissue}.{mode}.genes.tpm.tsv`: total TPM, one column per mouse.
- `{tissue}.{mode}.genes.founder_tpm.tsv`: long form with the per-founder TPM
...
for mouse, path in zip(mice, paths):
    ...
    totals.append(df.select(pl.col("locus").alias("gene_id"), pl.col("total").alias(mouse)))
    by_founder.append(
        df.select(
...
total_table.write_csv(snakemake.output.total, separator="\t")

pl.concat(by_founder).sort(["gene_id", "mouse_id"]).write_csv(
    snakemake.output.by_founder, separator="\t"
)
