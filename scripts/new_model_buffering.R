...
fit_model <- function(au) {
    ...
    family = binomial
    res_binom <- glmmTMB(
        cbind(haplotype_1_unique, haplotype_2_unique) ~ 0 + effect_A + effect_B + effect_C + effect_D + effect_E + effect_F + effect_G + (1 | mouse_id),  # H is reference
        family = family,
        data = au2,
    )
    binom_sample_variance <- VarCorr(res_binom)$cond$mouse_id[,'(Intercept)']
    res_null_binom <- glmmTMB(
        cbind(haplotype_1_unique, haplotype_2_unique) ~ 0 + (1 | mouse_id),  # H is reference
        family = family,
        data = au2,
    )
    anova_binom <- anova(res_binom, res_null_binom)
    ...
    results <- tibble(
        ...
        anova_binom_p = anova_binom[2,'Pr(>Chisq)'],
        anova_binom_chisq = anova_binom[2,'Chisq'],
        effect_A = cis_A,
        effect_B = cis_B,
        effect_C = cis_C,
        effect_D = cis_D,
        ...
        binom_sample_var = binom_sample_variance,
